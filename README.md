...
The portal tab is rebuilt on **startup** and after every successful **add/update** into a single styled page: a welcome + concept presentation (tiers, performance, build types, disclaimers, how submissions work, credits) followed by a tracks table. Table columns: `Track | Surface | Type | Meta focus | Last updated | Last submission (class, car, time, driver, date)` (the Track column links to its sheet). The whole page is generated by the bot, so do **not** hand-edit it (changes are overwritten). Track links use API hyperlinks (not `HYPERLINK` formulas) so they work on French-locale spreadsheets too. The "last submission" data comes from each track's `_SubmissionLog` (below). If the portal is not configured, the feature is silently disabled (no regression). Portal write failures are logged but never affect command replies.
